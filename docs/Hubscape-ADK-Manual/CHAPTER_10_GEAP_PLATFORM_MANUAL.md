...
* **Developer Configuration (`deploy_config.json`):** Developer-defined custom configurations (under the `create_params` key) are stored in `deploy_config.json` in the root of the repository. This file is Git-tracked and preserved across updates.
* **Directory Configuration (`agents-cli-manifest.yaml`):** The deployment configuration is declared in `agents-cli-manifest.yaml` in the root of the repository. During deployment, parameters from `deploy_config.json` are automatically merged into this file by `deploy.py`.
...
### Developer Configuration Profiles (`deploy_config.json`)
...
* **`app/config.json` Grounding Toggles**: Agent developers can enable native Google search and maps grounding by setting boolean flags in `app/config.json`:
  ```json
  {
